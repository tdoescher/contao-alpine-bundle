...
class AlpineJSFormFieldsCallback
{
    ...
    public function __invoke(DataContainer|null $dc = null): void
    {
        ...
        if (in_array($field->type, [ 'text', 'textdigit', 'textcustom', 'password', 'passwordcustom', 'textarea', 'textareacustom' ])) {
            PaletteManipulator::create()
                ->addLegend('alpinejsWidget_legend', 'fconfig_legend')
                ->addField('xInit', 'alpinejsWidget_legend', PaletteManipulator::POSITION_APPEND)
                ->addField('xShow', 'alpinejsWidget_legend', PaletteManipulator::POSITION_APPEND)
                ...
                ->addField('xRef', 'alpinejsField_legend', PaletteManipulator::POSITION_APPEND)
        ...
        if (in_array($field->type, [ 'select', 'radio', 'checkbox', 'upload', 'range' ])) {
            PaletteManipulator::create()
                ->addLegend('alpinejsWidget_legend', 'fconfig_legend')
                ->addField('xInit', 'alpinejsWidget_legend', PaletteManipulator::POSITION_APPEND)
                ->addField('xShow', 'alpinejsWidget_legend', PaletteManipulator::POSITION_APPEND)
            ...
            $GLOBALS['TL_DCA']['tl_form_field']['fields']['xOnChange']['eval']['tl_class'] = 'clr w100';
        }

        if (in_array($field->type, [ 'hidden', 'hiddencustom' ])) {
            PaletteManipulator::create()
                ->addLegend('alpinejsWidget_legend', 'fconfig_legend')
                ->addField('xInit', 'alpinejsWidget_legend', PaletteManipulator::POSITION_APPEND)
                ->addLegend('alpinejsField_legend', 'alpinejsWidget_legend')
            ...
            $GLOBALS['TL_DCA']['tl_form_field']['fields']['xInit']['eval']['tl_class'] = 'clr w100';
        }
        ...
        if ($field->type === 'submit') {
            ...
            $GLOBALS['TL_DCA']['tl_form_field']['fields']['xBindDisabled']['eval']['tl_class'] = 'clr w100';
        }
    }
}
